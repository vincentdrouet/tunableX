...
def main(argv=None):
    ...
    s.add_argument("--sys-path", dest="sys_paths", nargs="+", default=[],
                   help="Paths to insert into sys.path before imports.")
    ...
    t.add_argument("--sys-path", dest="sys_paths", nargs="+", default=[],
                   help="Paths to insert into sys.path before imports.")
    ...
    if args.cmd == "schema":
        ...
        if args.out: write_schema(args.out, schema, defaults)
        else: print(json.dumps({"schema": schema, "defaults": defaults}, indent=2))
        return 0
    ...
    if args.cmd == "trace":
        ...
        if args.out: write_schema(args.out, schema, defaults)
        else: print(json.dumps({"schema": schema, "defaults": defaults, "touched": sorted(touched)}, indent=2))
        return 0
