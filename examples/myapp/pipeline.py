...
@tunable("dropna","normalize","clip_outliers", namespace="preprocess", apps=("train","serve"))
def preprocess(path: str, dropna: bool=True, normalize: Literal["zscore","minmax","none"]="zscore",
...
def build_model(hidden_units: int=Field(128, ge=1), dropout: float=Field(0.2, ge=0.0, le=1.0)):
    print("build_model", hidden_units, dropout)
    return "model"

@tunable("epochs","batch_size","optimizer", namespace="train", apps=("train",))
def train(epochs: int=Field(10, ge=1), batch_size: int=32, optimizer: Literal["adam","sgd"]="adam"):
...
def train_main():
    preprocess("/data/train.csv"); build_model(); train()

def serve_main():
    preprocess("/data/live.csv"); serve_api()
